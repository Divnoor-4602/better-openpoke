---
name: deslop-review
description: Run a full-stack deslop review of OpenPoke frontend (`apps/web/`) and backend (`server/`) changes using all relevant installed skills. Use when asked to review uncommitted changes, current branch commits, recent commits, or catch sloppy/anti-pattern code before it lands.
metadata:
  scope:
    - apps/web/
    - server/
...
Use this skill when the user asks for a deslop review, broad code quality review, or review of uncommitted/recent OpenPoke changes.

Arguments/defaults for non-Claude agents:

- `days`: default `7` unless the user specifies another time window.
- `base`: default `main` unless the user specifies another base branch.
...
2. **Current branch commits** touching the path, ahead of the base branch (default `main`):
   - `git log --no-merges <base>..HEAD -- <path>/`
   - `git diff <base>...HEAD -- <path>/`

3. **Recent commits on this branch from the selected time window** (default `7`):
   - `git log --since="<days> days ago" --no-merges -- <path>/`
