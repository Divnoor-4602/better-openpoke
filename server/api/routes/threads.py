...
async def stream_thread_message(
    ...
    return StreamingResponse(
        runtime.stream_execute(user_content),
...
def create_thread_agent_run(
    ...
    request_id = payload.requestId or __import__("uuid").uuid4().__str__()
    ...
    if run is None:  # pragma: no cover - defensive
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Agent run not found")
